...
class Quick_And_Easy_FAQs_Public {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'font-awesome', plugin_dir_theme_url(__FILE__) . 'css/css/font-awesome.min.css', array(), $this->version, 'all' );
		wp_enqueue_style( $this->plugin_name, plugin_dir_theme_url(__FILE__) . 'css/quick-and-easy-faqs-public.css', array(), $this->version, 'all' );

        // if rtl is enabled
        if ( is_rtl() ) {
            wp_enqueue_style( $this->plugin_name . '-rtl', plugin_dir_theme_url(__FILE__) . 'css/quick-and-easy-faqs-public-rtl.css', array( $this->plugin_name, 'font-awesome' ), $this->version, 'all' );
        }
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_theme_url(__FILE__) . 'js/quick-and-easy-faqs-public.js', array( 'jquery' ), $this->version, false );
    ...
    private function toggles_for_all_faqs( $filter_array ) {
        ...
        if ( $faqs_query->have_posts() ) :
            while ( $faqs_query->have_posts() ) :
                $faqs_query->the_post();
                $fp = get_post( get_the_ID() );                
                ?>
                <div class="qe-faq-toggle">
                    <div class="qe-toggle-title" id="data-<?php echo $fp->post_name ?>">
